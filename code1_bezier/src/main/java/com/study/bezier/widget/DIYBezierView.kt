...
class DIYBezierView : BaseView {
    ...
    // 选中的点集合，受 status 影响
    private val mCurSelectPointList: MutableList<PointF> =
        ArrayList()
    ...
    // 有效触碰的范围
    private var mTouchRegionWidth:Float = 20.dpToPx()

    // 线的宽度
    private var LINE_WIDTH:Float = 2.dpToPx()

    // 控制点的半径
    private var POINT_RADIO_WIDTH:Float = 4.dpToPx()

    // 选中控制点的半径
    private var SEL_POINT_RADIO_WIDTH:Float = 6.dpToPx()

    // 拽动状态
    private var mStatus: Status? = null

    // 是否显示辅助线
    private var mIsShowHelpLine = false
    ...
    fun setIsShowHelpLine(isShowHelpLine: Boolean) {
        mIsShowHelpLine = isShowHelpLine
        invalidate()
    }
    ...
    override fun init(context: Context?) {
        ...
        mCirclePaint!!.strokeWidth = LINE_WIDTH
    ...
    private fun selectControlPoint(x: Float, y: Float): Boolean {
        ...
        when (mStatus) {
            Status.FREE -> mCurSelectPointList.add(
                mControlPointList!![selIndex]
            )
            Status.THREE -> {
            ...
            Status.MIRROR_DIFF, Status.MIRROR_SAME -> if (selIndex == 0 || selIndex == 6) {
    ...
    companion object {
        private const val BEZIER_CIRCLE_COLOR = "#20A298" //绿色
        private const val NATIVE_CIRCLE_COLOR = "#F6A010" //橙色
        private const val CONTROL_LINE_COLOR = "#FA3096" //艳红色
        private const val SEL_POINT_COLOR = "#1296db" //蓝色
    }
}